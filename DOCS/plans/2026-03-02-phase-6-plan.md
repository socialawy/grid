...
- Copy both files to project root / scripts:
  - build.js          → E:\co\GRID\build.js
  - migrate-to-build.ps1 → E:\co\GRID\scripts\migrate-to-build.ps1
...
The script now checks that build.js exists at the root before doing anything — if it's missing it errors cleanly with instructions rather than failing mid-way through extraction.

```bash
PS E:\co\GRID> .\scripts\migrate-to-build.ps1
...
==================================================
  ...
  -> Creating src/shell/...
  OK src/shell/ ready
  -> Extracting src/shell/head.html (lines 4-22)...
  OK head.html -- 19 lines
  -> Extracting src/shell/style.css (lines 24-571)...
  OK style.css -- 548 lines
  -> Extracting src/shell/body.html (lines 575-873)...
  OK body.html -- 299 lines
  -> Extracting src/shell/app.js (lines 2060-3152)...
  OK app.js -- 1093 lines
  -> Updating package.json...
  OK package.json updated (added: build, dev)

[GRID] Running node build.js...
[GRID] Building dist/index.html...   Γ£ô  dist/index.html ΓÇö 6403 lines, 208.1 KB   Γ£ô  Modules inlined: 13/13
  OK Build succeeded

[GRID] Migration complete
==================================================
  Created:
    src/shell/head.html   (19 lines)
    src/shell/style.css   (548 lines)
    src/shell/body.html   (299 lines)
    src/shell/app.js      (1093 lines)

  Next steps:
    1. Open dist/index.html in browser -- verify identical appearance
    2. node tests/run-all.js -- verify 661 pass, 0 fail
    3. If both pass: dist/index.html is now generated, not hand-edited
    4. Use 'node build.js' or 'npm run build' before every commit

  Do not manually edit dist/index.html after this point.
  Edit src/shell/app.js or src/shell/style.css instead.
```
...
## Follow up on -- `docs\plans\2026-03-02-build-and-phase-6-implementation.md`

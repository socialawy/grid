...
if (isNode) {
  ...
  // Load grid-core from src as CommonJS
  const corePath = path.join(__dirname, '..', 'src', 'core', 'grid-core.js');
  delete require.cache[require.resolve(corePath)];
  const moduleExports = require(corePath);
  GridCore = moduleExports.default || moduleExports;
} else {
  // Browser environment - GridCore should be global
  GridCore = window.GridCore;
}
